...
pred.mm1 <- ggpredict(lmer_coi_readSpikeCorr5, 
                      terms = c("log.readNumSpikeCorr [all]"))
...
pred.mm2 <- ggpredict(lmer_coi_read5, terms = c("log.readNum [all]"))
...
#
pred.mm3 <- ggpredict(lmer_gDNA_readSpikeCorr5, 
                      terms = c("log.readNumSpikeCorr [all]"))
#
(p3 <- ggplot() +
  geom_line(
...
pred.mm5 <- ggpredict(lmer_gDNA_UMI5, terms = c("log.mtagNum [all]"))
...
pred.mm6 <- ggpredict(lmer_gDNA_UMIspikeCorr5, 
                      terms = c("log.mtagNumSpikeCorr[all]"))
...
pred.mm7 <- ggpredict(lmer_COI_UMIspike5, terms = c("log.mtagNum [all]"))
...
pred.mm8 <- ggpredict(lmer_COI_UMIspikeCorr5, 
                      terms = c("log.mtagNumSpikeCorr [all]"))
...
#### Test of Warton's spike-in estimator on full dataset, with OTUID, soupRep, and pcrRep   

Upshot 1:  Warton's offset estimator is not as good as a physical spike-in but better than not having it. With the offset estimator, the fixed-effect R2 = 10.3% for predicting input COI copy number (vs. 4.2% without) and R2 = 18.1% for predicting input genomic DNA (vs. 0.5% without). This is with 63 samples (= 7 soups X 3 soup replicates X 3 PCR replicates per soup) and 52 OTUs  
